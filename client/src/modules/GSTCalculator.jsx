...
export default function GSTCalculator({ gstExpenses, setGstExpenses, gstImpact }) {
  ...
  return (
    <div className="space-y-8">
      <div><h2 className="text-2xl font-bold">GST Impact Calculator</h2><p className="text-slate-400">See GST on monthly expenses (GST 2.0 - Sept 2025)</p></div>

      <div className="grid md:grid-cols-4 gap-4">
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6"><span className="text-3xl">🛒</span><p className="text-sm text-slate-400 mt-2">Total Spending</p><p className="text-2xl font-bold">{formatCurrency(gstImpact.totalExpense)}</p></div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6"><span className="text-3xl">📊</span><p className="text-sm text-slate-400 mt-2">Effective GST</p><p className="text-2xl font-bold">{gstImpact.effectiveGSTRate.toFixed(1)}%</p></div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6">
          ...
          <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
            {gstExpenses.map((expense, i) => (
              <div key={i} className="p-4 rounded-xl bg-slate-800">
                ...
                <div className="flex items-center gap-3">
                  <input type="range" min="0" max={expense.avgMonthly * 3} step="100" value={expense.amount}
                    onChange={(e) => { const n = [...gstExpenses]; n[i].amount = parseInt(e.target.value); setGstExpenses(n); }} className="flex-1 accent-emerald-500" />
                  <input type="number" value={expense.amount}
                    ...
                    className="w-24 px-3 py-1 rounded-lg bg-slate-700 border border-slate-600 text-right font-mono text-sm outline-none" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={(value) => formatCurrency(value)} contentStyle={{ backgroundColor: '#1e293b', border: 'none', borderRadius: '12px' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
