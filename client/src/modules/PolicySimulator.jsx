...
export default function PolicySimulator({ taxInputs, gstImpact, newRegimeTax }) {
  ...
  // AI Logic (Uses the original specific route for sliders)
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-8">
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            ...
            <button onClick={() => setPolicySimulation({ gstChange: 0, incomeTaxChange: 0, fuelTaxChange: 0 })} className="w-full py-3 rounded-xl bg-slate-800 hover:bg-slate-700 transition-colors text-sm font-bold">Reset to Default</button>
          </div>

          {/* Impact Visualization */}
          <div className="space-y-6">
            <h3 className="text-xl font-semibold flex items-center gap-2">🎯 Projected Impact</h3>
            <div className="p-6 rounded-2xl bg-slate-800 border border-slate-700">
               <h4 className="font-semibold mb-4 text-slate-300">Monthly Cost Change</h4>
               <div className="space-y-4">
                  <div className="flex justify-between text-sm"><span>🛒 GST Impact</span><span>{formatCurrency(Math.round(policySimulation.gstChange * gstImpact.totalGST / 100))}</span></div>
                  <div className="flex justify-between text-sm"><span>💰 Tax Impact</span><span>{formatCurrency(Math.round(policySimulation.incomeTaxChange * newRegimeTax.totalTax / 100 / 12))}</span></div>
                  <div className="flex justify-between text-sm"><span>⛽ Fuel Impact</span><span>{formatCurrency(policySimulation.fuelTaxChange * 100)}</span></div>
                  ...
                  <div className="border-t border-slate-600 pt-4 mt-2">
                    {(() => {
                      ...
                      return <div className="flex justify-between items-center"><span className="font-bold text-white">Net Monthly Impact</span><span className={`font-mono text-xl font-bold ${total > 0 ? 'text-red-400' : total < 0 ? 'text-emerald-400' : 'text-slate-400'}`}>{total > 0 ? '+' : ''}{formatCurrency(total)}</span></div>;
                    })()}
                  </div>
               </div>
            </div>
            ...
            <div className="pt-2">
                ...
                {aiExplanation && (
                  <div className="mt-4 p-4 bg-slate-800 rounded-xl border border-indigo-500/30">
